import { Settings2, KeyRound, Trash2 } from 'lucide-react';

export function AdminUsersTable({ users, loading, userObj, handleOpenModules, handleOpenPassword, handleDeleteUser }: any) {
  return (
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50 text-slate-700 dark:text-slate-300">
            ...
            {users.length === 0 && !loading && (
              <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-500 font-bold text-lg">No hay usuarios en la red.</td></tr>
            )}
          </tbody>
        </table>
      </div>
